...
[`docs/analyse-rgpd.md`](https://github.com/Oumbra/wakfu-companion/blob/main/docs/analyse-rgpd.md)
...
| C25 | « Deux fonctions optionnelles » pour trois fonctions, dont F1/F2 non désactivables. Case « Activer les raccourcis multicompte » (onglet « Raccourcis »), **décochée par défaut** même pour une configuration existante (décision du mainteneur) ; « Trois fonctions » dans « À propos » et le README | `shortcuts.rs`, `config.rs::multiaccount_shortcuts`, `panels/raccourcis_tab.rs` |

**Site déployé** (confirmé par le mainteneur le 2026-09-25) : C5 (rotation du jeton) et C9
(`PATCH` partiel), qui en dépendaient, sont actifs en production ; les mentions « dépend du
déploiement » ci-dessous sont retirées et le lien de l'en-tête vise `main`.
...
| C5 | **Rotation du jeton natif appelée** : au démarrage, jeton accepté et file d'envoi pas encore activée, `POST /api/v1/auth/native/session` s'il a plus de 7 jours ou si sa date d'émission est inconnue ; nouveau jeton écrit au trousseau avant d'être utilisé, échec = ancien jeton conservé, jamais une déconnexion. Date d'émission dans `native-session.issued-at` (posée par `save_token`, effacée par `clear_token`). Actif en production depuis le déploiement du site (confirmé le 2026-09-25) | `overlay-ui/src/background.rs::rotate_token_if_due`, `overlay-sync/src/client.rs::rotate_native_session`, `token_store.rs::token_age` |
| C9 | **Écriture partielle** de `PATCH /api/v1/settings` : `profile` ne part plus qu'avec ses trois champs d'alerte (`patch`, fusion côté serveur), `roster` avec les seuls comptes modifiés ou créés et les `removedIds` (écart avec le roster connu, `Roster::patch_against`) ; `profile_raw` et `RosterAccount::extra` retirés, le pseudo et l'avatar ne transitent plus par l'overlay ; un refus (`rejected`) est journalisé, la version du compte l'emporte. Actif en production depuis le déploiement du site (confirmé le 2026-09-25) | `overlay-engine/src/profile.rs::patch_fields`, `roster.rs::patch_against`, `overlay-sync/src/client.rs::patch_settings` |
...
- C5 : purge à la déconnexion, bouton « Supprimer les données locales », appel de
  `DELETE /api/v1/auth/native/session` et rotation du jeton au démarrage passé 7 jours.
