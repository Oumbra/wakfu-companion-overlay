...
Chaque panneau est déplaçable (voir §6.4, poignée de glissement), avec opacité réglable ;
disposition persistée par écran (fait, L2). Pas de redimensionnement manuel ni de thème
configurable — voir la décision ci-dessous.
...
| **L2 — UI** 🟡 en cours | dégâts, suivi, alertes, récap, disposition persistée | Utilisable en jeu une soirée sans redémarrage — **fait** : `crates/overlay-engine/` (QuickJS + `LogParser` vendu → `LogEntry` → `SessionSnapshot`, + `watchlist.rs` — comptage du Suivi et alertes de décompte portés en Rust, voir §14 point 3) et `crates/overlay-ui/` (fenêtre S1, deux fenêtres overlay indépendantes Combat/Suivi — bande de tuiles, alertes son+toast sur décompte à 0), validés sur un vrai `wakfu.log`. **Fait (2026-09-02, suite)** : disposition persistée par écran (§6.4) — poignée de glissement « ⠿ » (visible en mode INTERACTIF, `egui::Sense::drag()`) sur chaque panneau, décalage résultant ajouté à l'ancrage automatique (`App::anchor_position`) et persisté par écran (`layout_store`, clé `MonitorHandle::name()`) — réappliqué après redémarrage. Interaction de glissement en elle-même **non vérifiée visuellement en conditions réelles** (build/clippy/tests propres, mais pas de fenêtre de jeu disponible ici pour la déclencher à la souris) — à confirmer au prochain lancement en jeu. **Fait (2026-09-02, suite)** : Alertes de drop version « ramassage avec son activé » — `overlay_engine::profile` lit `data.profile.soundItems` (`GET /api/v1/settings`), indépendant de la watchlist ; `Engine::drain_loot_alerts` déclenche toast + son (`alert_sound::play_loot_alert`, fichier mp3 identique au web) pour tout objet ramassé dont le son est activé au compte, suivi ou non — miroir de `registerLoot`/`ProfileService.findEnabledSoundItem`. **Reste** : État de synchro (dépend de L5). **Retiré (2026-09-02, décision du mainteneur, voir §9)** : thème configurable/mode daltonien — un overlay n'est pas un site, palette fixe assumée |
