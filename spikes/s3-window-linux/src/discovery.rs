...
//! `crates/overlay-ui/src/game_window.rs` (Windows), voir sa doc de module pour le contexte complet
//! (multi-compte, `" - WAKFU"` comme seul discriminant fiable puisque le process est `java`
//! générique). Ce module est l'ébauche directe de `overlay-platform::linux::x11` (§17.2 du plan) :
//! mêmes types (`GameRect`, `GameWindowInfo`, `scan()` renvoyant `Vec<(String, GameWindowInfo)>`)
//! pour que la migration hors de `spikes/` (critère de sortie de S3, §12) reste une simple copie de
//! fichier plutôt qu'une réécriture.
//!
