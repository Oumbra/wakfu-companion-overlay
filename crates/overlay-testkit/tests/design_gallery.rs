...
fn section_autocomplete(ui: &mut egui::Ui) {
    ...
    heading(
        ...
        "Neuf entrées, cinq visibles : la barre du web (8 px, rayon 4) sur un rail plus sombre que la liste, qui la déborde d’un pixel de chaque côté et en butée — la poignée, de la teinte de la liste au repos, s’y lit toujours. Elle ne s’élargit pas ; sous le pointeur, elle prend la teinte des rangées survolées. Sa colonne est réservée : la mention de droite ne passe jamais dessous. Les flèches font défiler la liste pour garder l’entrée active en vue.",
    );
